...
1. 保持 V1 文档和旧数据生成器不变，新建 V2 转换器。
2. 先支持四麻标准局内事件：`tsumo`、`dahai`、`chi`、`pon`、`daiminkan`、`ankan`、`kakan`、`reach`。
3. `start_game`、`start_kyoku`、`end_kyoku`、`end_game` 由环境侧处理；其中 `start_kyoku.kyoku` 映射为 `STATE_KYOKU_INDEX`。
4. 决策训练样本应同时保存 V2 输入序列和环境侧合法动作集合，但合法动作集合不编码为 V2 token。
5. 模型输出可以暂时继续使用现有离散动作空间；环境负责把输出映射到当前合法动作并处理 stale/rejected/defaulted。
6. 数据校验器应检查状态区和事件区一致性，尤其是手牌计数、副露、宝牌数量、立直状态和剩余牌数。
